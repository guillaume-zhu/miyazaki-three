...
export function createGrass(scene, platformMesh) {
  ...
  function isInVisibleArc(x, z) {
    ...
    while (diff < -Math.PI) diff += Math.PI * 2
    return Math.abs(diff) < VISIBLE_ARC / 2
  }
  ...
  function getSurfaceY(x, z) {
    raycaster.set(new THREE.Vector3(x, RAY_ORIGIN_Y, z), rayDir)
  ...
  const validOffsets=[], validScales=[], validRotations=[], validLeans=[]
  let placed = 0
  ...
  for (let i = 0; i < maxAttempts && placed < BLADE_COUNT; i++) {
    ...
    let scale = bumpScale*individualRandom
    scale *= 1.0-Math.pow(radius/RADIUS,3)*0.5
    validScales.push(scale)
    validRotations.push(Math.random()*Math.PI)
    const lean = (Math.random()-0.5)+(Math.random()-0.5)
    validLeans.push(lean)
    placed++
  }
  ...
  instancedGeo.setAttribute('aOffset',   new THREE.InstancedBufferAttribute(new Float32Array(validOffsets),   3))
  instancedGeo.setAttribute('aScale',    new THREE.InstancedBufferAttribute(new Float32Array(validScales),    1))
  instancedGeo.setAttribute('aRotation', new THREE.InstancedBufferAttribute(new Float32Array(validRotations), 1))
  instancedGeo.setAttribute('aLean',     new THREE.InstancedBufferAttribute(new Float32Array(validLeans),     1))
  ...
  const material = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float leanCurve = pow(uv.y, 1.8);
        float leanAmount = aLean * 0.45;
        pos.x += sin(aRotation) * leanAmount * leanCurve;
        pos.z += cos(aRotation) * leanAmount * leanCurve;
        ...
        pos += aOffset;
        vHeight   = uv.y;
        vWindBend = windForce * bendFactor;
        vWorldPos = aOffset;
